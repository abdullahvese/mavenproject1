...
public class FirstAutomation {
    public static void main(String[] args) {
     ...
     //add request
       String response= given().log().all().queryParam("key","qaclick123").header("Content-Type","application/json").body(Body.bodyTag()).when().post("maps/api/place/add/json").then().log().all().assertThat().statusCode(200).body("scope", equalTo("APP")).extract().response().asString();//extract.response.asString responsu tek satırda yaszdırır.
